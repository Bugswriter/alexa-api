...
class CRUDSpot():
    ...
    def create_spot(self, db: Session, spot: SchemaSpotCreate):
        db_spot = Spot(name= spot.name, location= spot.location, info= spot.info, special_attraction= spot.special_attraction, things_to_do= spot.things_to_do, time_to_visit= spot.time_to_visit, near_by_places= spot.near_by_places, similar_places= spot.similar_places, how_to_reach= spot.how_to_reach)
        db.add(db_spot)
        db.commit()
        db.refresh(db_spot)
        return db_spot
